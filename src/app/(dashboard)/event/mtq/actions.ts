...
export async function getMtqRegistrations() {
  noStore();
  try {
    // 1. Get the MTQ event ID
    const { data: eventData } = await supabaseServer
      .from("events")
      .select("id")
      .eq("slug", "lomba-keagamaan")
      .single();

    if (!eventData) {
      return { success: false, data: [], message: "Event lomba-keagamaan tidak ditemukan di Supabase." };
    }
    ...
    const formattedParticipants = registrations?.map((reg: any) => {
      ...
      // MTQ is free, but we still handle transaction status safely
      let paymentStatus = "Gratis";
    ...
    console.error("Failed to fetch MTQ registrations:", error);
    return { success: false, data: [], message: error.message };
  }
}
