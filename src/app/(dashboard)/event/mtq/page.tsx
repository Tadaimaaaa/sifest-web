...
import { 
  Users, Calendar, Clock, MapPin, Search, Plus, Loader2, ArrowLeft, ArrowUpDown, Edit3, X, Save
} from "lucide-react";
...
export default function MtqDashboard() {
  ...
  const hasAccess = ["ROLE-001", "SUPER_ADMIN", "ROLE-010"].includes(currentUserRole);

  const fetchData = async () => {
    // 1. Fetch Event Info
    try {
      const resEvent = await fetch(`${SCRIPT_URL}?action=getEvent&id_event=mtq`);
      ...
      console.error("Gagal mengambil info event MTQ:", error);
    }

    // 2. Fetch Participants from Supabase (Official Web Registrations)
    try {
      const dataParticipants = await getMtqRegistrations();
      if (dataParticipants.success && dataParticipants.data) {
        setParticipants(dataParticipants.data);
        setErrorMessage(null);
      } else {
        setErrorMessage(dataParticipants.message || "Gagal memuat data pendaftar MTQ");
        toast.error(dataParticipants.message || "Gagal memuat data pendaftar MTQ");
      }
    } catch (error: any) {
      setErrorMessage(error.message || "Terjadi kesalahan sistem");
      console.error("Gagal mengambil data peserta MTQ:", error);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const getStatusColor = (status: string) => {
    switch(status?.toLowerCase()) {
      case 'lunas':
      case 'gratis': return 'bg-emerald-100 text-emerald-700 border-emerald-200';
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-slate-500 mt-1">
              Kelola data peserta Lomba Keagamaan MTQ SI FEST 2026
            </p>
          </div>
        </div>
      ...
      <div className="relative">
        <div className="flex justify-between items-center mb-4">
          ...
          {hasAccess && (
            <button 
              ...
              className="flex items-center gap-2 px-3 py-1.5 bg-indigo-50 text-indigo-700 hover:bg-indigo-100 rounded-lg text-sm font-semibold transition-colors"
            >
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm relative overflow-hidden group">
            <div className="absolute -right-6 -top-6 w-24 h-24 bg-indigo-50 rounded-full group-hover:scale-150 transition-transform duration-500" />
            <div className="relative">
          ...
          <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm">
            ...
            <h3 className="text-base font-bold text-slate-800 mt-1">{eventData?.tanggal || "27 Oktober 2026"}</h3>
          </div>
          ...
          <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm">
            ...
            <h3 className="text-base font-bold text-slate-800 mt-1 truncate" title={eventData?.tempat || eventData?.lokasi || "Masjid Raya UPI YPTK"}>
              {eventData?.tempat || eventData?.lokasi || "Masjid Raya UPI YPTK"}
            </h3>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        ...
        <div className="p-5 border-b border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-bold text-slate-800">Daftar Peserta MTQ</h2>
          ...
          <div className="flex items-center gap-3">
            <div className="relative">
              ...
              <input 
                ...
                className="pl-9 pr-4 py-2 rounded-xl border border-slate-200 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 w-full sm:w-64 transition-all"
              />
            </div>
          </div>
        </div>
            ...
            <form onSubmit={handleSaveEvent} className="p-6 space-y-4">
              ...
              <div>
                ...
                <input 
                  ...
                  placeholder="Contoh: Masjid Raya UPI YPTK"
                  className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm text-slate-900 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500"
                />
              </div>
              ...
              <div className="pt-4 mt-6 border-t border-slate-100 flex justify-end gap-3">
                ...
                <button 
                  type="submit"
                  disabled={isSaving}
                  className="px-4 py-2 text-sm font-semibold text-white bg-indigo-600 hover:bg-indigo-700 rounded-xl transition-colors flex items-center gap-2 disabled:opacity-70"
                >
