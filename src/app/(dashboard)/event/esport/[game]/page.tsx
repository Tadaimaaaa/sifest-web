...
export default function EsportDashboard() {
  ...
  const gameSlug = game === 'mlbb' ? 'turnamen-esport-mlbb' : 'turnamen-esport-efootball';
  ...
  const [eventData, setEventData] = useState<EventData>({
    id_event: "esport",
    nama_event: game === 'mlbb' ? "Mobile Legends: Bang Bang" : "E-Football",
    tanggal: "",
    tempat: "",
    deskripsi: "",
    status: "Akan Datang"
  ...
  const fetchData = async () => {
    try {
      ...
      const dataTeams = await getEsportRegistrations(gameSlug);
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-slate-100">
              {teams.length === 0 ? (
                <tr>
                  <td colSpan={game === 'efootball' ? 5 : 4} className="px-6 py-12 text-center">
