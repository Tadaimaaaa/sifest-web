...
export default function ProdukDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const BUNDLES: BundleType[] = [
    ...
    { id: 'luar-kota', nama: 'Harga Luar Kota', harga: 12000, isDynamic: true, minItems: 1 },
    { id: 'grosir', nama: 'Harga Grosir', harga: 11000, isDynamic: true, minItems: 1 },
  ];
            ...
            <div className="p-4 border-b border-slate-100 flex items-center justify-between bg-slate-50">
              ...
              {hasAccess && (
                <button
                  onClick={() => {
                    if (isByU) {
                      setVarianFormData({ nama_varian: "Kartu by.U", jumlah: "", fileData: "", fileName: "", mimeType: "" });
                    }
                ...
                >
                  <Plus className="w-3.5 h-3.5" /> {isByU ? 'Tambah Stok' : 'Tambah Varian'}
                </button>
              )}
            </div>
                ...
                {!produk.varian || produk.varian.length === 0 ? (
                  ...
                  <div className="space-y-3">
                    {produk.varian.map((varItem) => {
                      ...
                      const maxStock = getMaxStockForVarian(varItem.id_varian);
                      const totalSelected = Object.values(bundleItems).reduce((sum, val) => sum + val, 0);
